...
const getLoginToken = (userType = 'admin') => {
  return new Promise((resolve, reject) => {
    chai
      .request(server)
      .post('/auth/login')
      .send(loginDetails[userType])
      .end((err, res) => {
...
describe('*********** USERS ***********', () => {
  ...
  // before(async()=> {
  //   tokens.admin = await getLoginToken();
  //   tokens.user = await getLoginToken("user");
  // })
  ...
  describe('/GET users', () => {
    ...
    it('it should GET the users with filters', (done) => {
      chai
        .request(server)
        .get('/users?filter={"name": { "$regex": "admin", "$options":"i"}}')
        .set('Authorization', `Bearer ${tokens.admin}`)
        .end((err, res) => {
  ...
  describe('/GET/:id user', () => {
    it('it should GET a user by the given id', (done) => {
      const id = '5aa1c2c35ef7a4e97b5e0003';
        ...
        .end((err, res) => {
          ...
          chai
            ...
            .end((error, result) => {
              ...
              result.body.should.have.property('meta').eql('DELETED');
              done();
            });
        });
    });
