...
const FnFeedsClass = require('../../feeds/del/functions');
...
const getCompetitions = () => {
  return MAINCONFIG.feeds.del.competitions;
};
...
const getMatches = async ({ cid, tid }) => {
  ...
  const matchesData = await FnFeeds.getMatches();
...
const getMatch = async ({ cid, tid, mid }) => {
  if (!tid || !mid) {
    return;
  }
  ...
  const matchday = await FnFeeds.getMatchday();

  return matchParser(matchday, tid);
};
