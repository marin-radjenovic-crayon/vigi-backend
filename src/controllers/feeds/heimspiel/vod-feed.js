...
const BaseClass = require('../opta/index');
...
const { getSinceDate } = require('@/src/controllers/feeds/heimspiel/helpers');
...
class VodFeed extends BaseClass {
  ...
  async getOlympicsVideos() {
    const indicator = config.feeds.opta.vodData.byKeywords;
    const apiCall = indicator.url
      .replace('[[KEYWORDS]]', 'kwd=Olympics')
      .replace('[[SINCE_DATE]]', getSinceDate(2 * 24));
    ...
    const maxDuration = 10 * 60; //(6 minutes)
