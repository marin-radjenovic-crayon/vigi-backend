 ...
 */
function getGoalData(events, lineUp, targetGoal, targetActionCount = 10) {
  const actionCnt = targetActionCount;
  const data = [];
  let team = null;
  let goalIndex = 1;
  ...
  for (let i = events?.length - 1; i >= 0; i--) {
    ...
    if (isOwnGoal) {
      if (team && data?.length < actionCnt) {
        ...
        team = lineUp.find(team => team.contestantId === contestantId);
        const opponentTeam = lineUp.find(team => team.contestantId !== contestantId);
      ...
      if (team && data?.length < actionCnt) {
        // if (team && contestantId === team?.contestantId && data?.length < 5) {
        if (skipEvents.includes(typeId)) {
          break;
        }
        if (typeId === deletedEventId) {
