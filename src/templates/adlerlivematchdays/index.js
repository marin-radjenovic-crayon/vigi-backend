...
Template.prototype.getDynamicData = async function (baseData) {
  ...
  const currentMatchday = await FeedInstance.getMatchday().catch(err => {
    console.log('err', err);
  });

  const currentRound = _.get(currentMatchday, 'round');

  const roundOverview = await FeedInstance.getRoundOverview(currentRound, event.selectedTeamId).catch(err => {
    console.log('err', err);
  });
  ...
  _.each(roundOverview, (item, index) => {
    teamsIds.push(item.home.id.toString());
    teamsIds.push(item.guest.id.toString());
    // teamsIds.push(item.guest.id.toString());
  });
