...
Template.prototype.getDynamicData = async function (baseData) {
  ...
  const target = ['faceoffs', 'puckcontrol', 'goals', 'shots', 'skaterstatistics', 'teamstatistics'];

  await FeedWise.setMatchStatistics(target).catch(err => {
    console.log('err', err);
  });

  const homePlayers = await FeedWise.getPlayers(event.homeTeamId).catch(err => {
    console.log('err', err);
  });

  const awayPlayers = await FeedWise.getPlayers(event.awayTeamId).catch(err => {
    console.log('err', err);
  });
  ...
  return {
    ...
    game,
    matchStats: FeedWise.apiData.liveGame.matchStats,
    players,
    assets
  };
};
