...
Template.prototype.getDynamicData = async function (baseData) {
  ...
  const gameType = _.find(baseData.data.fields, { id: 'gameType' })?.value || 'last';

  let lastGames = null;
  let lastHomeGames = null;
  let lastAwayGames = null;

  if (gameType === 'last') {
    lastHomeGames = await FeedInstance.getTeamLastGames('home').catch(err => {
      console.log('err', err);
    });
    lastAwayGames = await FeedInstance.getTeamLastGames('away').catch(err => {
      console.log('err', err);
    });
  } else {
    lastHomeGames = await FeedInstance.getTeamNextGames('home').catch(err => {
      console.log('err', err);
    });
    lastAwayGames = await FeedInstance.getTeamNextGames('away').catch(err => {
      console.log('err', err);
    });
  }
