...
Template.prototype.getDynamicData = async function (baseData) {
  ...
  const mvp = await FeedInstance.getMVP().catch(err => {
    console.log('err', err);
  });

  const winningTeamSide = mvp?.winningTimeIndex === 0 ? 'home' : 'away';
  const playerStats = await FeedInstance.getPlayerStatsById(mvp?.mvp?.playerId).catch(err => {
    console.log('err', err);
  });
  ...
  return {
    matchInfo: playerStats?.matchInfo,
    liveData: playerStats?.liveData,
