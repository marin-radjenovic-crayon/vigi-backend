  ...
  for (let i = events?.length - 1; i >= 0; i--) {
    ...
    if (isOwnGoal) {
      if (team && data?.length < 5) {
        if (typeId === foulEventTypeId || typeId === cardEventTypeId || typeId === outEventTypeId || typeId === cornerAwardedEventTypeId) { // Do not insert event if it is not regular game play
          break;
        }
        team = lineUp.find(team => team.contestantId === contestantId);
